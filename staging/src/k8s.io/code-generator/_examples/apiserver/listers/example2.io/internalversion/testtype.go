...
type TestTypeLister interface {
	...
	List(selector labels.Selector) (ret []*example2io.TestType, err error)
...
// List lists all TestTypes in the indexer.
func (s *testTypeLister) List(selector labels.Selector) (ret []*example2io.TestType, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*example2io.TestType))
	})
	return ret, err
}
...
type TestTypeNamespaceLister interface {
	...
	Get(name string) (*example2io.TestType, error)
...
// List lists all TestTypes in the indexer for a given namespace.
func (s testTypeNamespaceLister) List(selector labels.Selector) (ret []*example2io.TestType, err error) {
...
// Get retrieves the TestType from the indexer for a given namespace and name.
func (s testTypeNamespaceLister) Get(name string) (*example2io.TestType, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(example2io.Resource("testtype"), name)
	}
	return obj.(*example2io.TestType), nil
}
